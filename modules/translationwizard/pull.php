...
switch($mode) {
	...
	while (list($key,$val) = each ($pullmodules)) {
		$sql="";
...
default:
	...
	while (list($key,$val) = each($listing))
		...
		rawoutput("</td><td>");
		rawoutput("<a href='runmodule.php?module=translationwizard&op=pull&mode=pull&pulldir=".rawurlencode($selecteddir)."&mirror=".rawurlencode($mymirror)."&ns=".rawurlencode($val)."'>". translate_inline("Pull")."</a>");
		addnav("", "runmodule.php?module=translationwizard&op=pull&mode=pull&pulldir=".rawurlencode($selecteddir)."&mirror=".rawurlencode($mymirror)."&ns=".rawurlencode($val));
		rawoutput("</td><td>");
		rawoutput("<a href='runmodule.php?module=translationwizard&op=push&mode=push&ns=". rawurlencode($val)."'>". translate_inline("Push")."</a>");
		addnav("", "runmodule.php?module=translationwizard&op=push&mode=push&ns=". rawurlencode($val));
		rawoutput("</td><td>");
		rawoutput("<a href='runmodule.php?module=translationwizard&op=pull&pulldir=".rawurlencode($selecteddir)."&mirror=".rawurlencode($mymirror)."&getdate=1&ns=". rawurlencode($val)."'>". translate_inline("Pull Date")."</a>");
		addnav("", "runmodule.php?module=translationwizard&op=pull&pulldir=".rawurlencode($selecteddir)."&mirror=".rawurlencode($mymirror)."&getdate=1&ns=". rawurlencode($val));
		rawoutput("</td></tr>");
		}
		rawoutput("</table>");
		rawoutput("<input type='submit' name='pulluntranslated' value='". translate_inline("Pull only for those in the untranslated") ."' class='button'>");

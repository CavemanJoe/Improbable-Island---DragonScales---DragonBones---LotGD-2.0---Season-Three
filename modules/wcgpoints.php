		...
		case "newday":
			if (get_module_pref("wcgid")){
				...
				if ($newpoints > 1){
					...
					$args['allpoints'] = get_module_pref("points");
...
function wcgpoints_run(){
	...
	switch (httpget('op')){
		case "enter":
			...
			rawoutput("<iframe src=\"http://www.worldcommunitygrid.org/getDynamicImage.do?teamId=13F2W0053V1&mnOn=true&stat=4&imageNum=2&rankOn=true&projectsOn=false&special=false\" frameborder=\"0\" name=\"di\" scrolling=\"no\" width=\"405px\" height=\"145px\"></iframe>");
		...
		if (!$loaderror){
			...
			if (!$xmlObj){
				...
				if ($xmlObj->MemberStats){
					...
					$runtime = $xmlObj->MemberStat->StatisticsTotals->RunTime;
					if (!$runtime) $runtime = $xmlObj->MemberStats->MemberStat->StatisticsTotals->RunTime;
					if (!$runtime) $runtime = $xmlObj->MemberStatsWithTeamHistory->MemberStats->MemberStat->StatisticsTotals->RunTime;
					if (!$runtime) return false;
					$lastpoints = get_module_pref("points","wcgpoints");
					if ($points > $lastpoints){
						set_module_pref("runtime",$runtime,"wcgpoints");

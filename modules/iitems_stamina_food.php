...
function staminafood_dohook($hookname,$args){
	...
	switch($hookname){
			...
			if ($session['user']['race']!="Robot" && $session['user']['race']!="Gobot" && $session['user']['race']!="Foebot" && $session['user']['race']!="Joker" && $session['user']['race']!="Stranger"){
				...
				switch($session['user']['location']){
					...
					case "Kittania":
						...
						break;
					case "New Pittsburgh":
						addnav("Mutated Munchies","runmodule.php?module=staminafood&op=start&location=np");
						break;
					case "Squat Hole":
		...
		case "stamina-newday-intercept":
			if ($session['user']['race']!="Robot" && $session['user']['race']!="Gobot" && $session['user']['race']!="Foebot" && $session['user']['race']!="Joker" && $session['user']['race']!="Stranger" && $session['user']['age']>1){
				//Remove the value of used Stamina from the player's Nutrition and Fat counters
				require_once("modules/staminasystem/lib/lib.php");
				$pctused = round(100 - get_stamina(3));
				if ($pctused>0){
					set_module_pref("nutrition",get_module_pref("nutrition")-$pctused);
					set_module_pref("fat",get_module_pref("fat")-$pctused);
				}
...
function staminafood_run(){
	...
	if (get_module_pref("fullness")<=100){
		switch (httpget("op")){
			case "start":
				switch (httpget("location")){
					...
					case "np":
						...
						output("You head into the local cafe, imaginatively titled \"BRAAAAAINS\".  A waiter comes shuffling over to you, green skin peeling from his face.  \"BRAAAAAINS?\" he asks, holding out a menu.`n`n");
						if ($session['user']['gold']>=50){
			...
			case "buy":
				...
				switch (httpget("bought")){
					case 1:
						output("You munch happily on your crisps, reflecting that they're probably not too good for you - but hell, at least they're cheap.");
						$st = 5000;
						$nu = 5;
						$fa = 10;
						$fu = 5;
					...
					case 21:
						...
						break;
					case 22:
						output("You stare down at your three-eyed fish.  It stares back up at you, in ways that you can only imagine.  After a few moments depressed contemplation, you tuck in - and hey, it's actually not that bad.`n`nYou gain some Stamina!");
						$st = 305000;
						$nu = 50;
						$fa = 25;
						$fu = 40;
						...
						break;
					case 23:
						output("As your fork sinks into your Magical Mystery Meatloaf, a hundred pairs of eyes open on its crispy skin.  They close again almost instantly, and you try your hardest to persuade yourself that you're going insane - it beats reality, that's for damn sure.`n`nYou gain some Stamina!");
						$st = 385000;
						$nu = 50;
						$fa = 25;
						$fu = 50;
					...
					case 28:
						...
						break;
					case 29:
						output("Cock Nuggets turned out to be chicken nuggets, only with a little more fowl language.`n`nYou gain some Stamina!");
						$st = 145000;
						$nu = 30;
						$fa = 60;
						$fu = 25;
		...
		page_header("Too Full!");
		output("You are far too full to eat any more today.");
	}

...
function titlechange_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"take"=>"Actually remove points when purchasing a title?,bool|1", // change by sixf00t4.
			...
			"permanent"=>"Cost of Unlimited Changes,bool|1000",
		),
		"prefs"=>array(
...
function titlechange_dohook($hookname,$args){
	...
	case "lodge":
		...
		if ($hasperm){
			...
			$cost = sprintf_translate("%s points", $cost);
			addnav(array("Set Custom Title (%s)", $cost),"runmodule.php?module=titlechange&op=titlechange");
			addnav(array("Get unlimited Title changes (%s points)", $costperm),"runmodule.php?module=titlechange&op=permanent");
		}

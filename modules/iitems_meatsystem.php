...
function iitems_meatsystem_getmoduleinfo(){
	$info = array(
		"name"=>"IItems Meat System",
...
function iitems_meatsystem_install(){
...
function iitems_meatsystem_uninstall(){
	require_once "modules/staminasystem/lib/lib.php";
...
function iitems_meatsystem_run(){
	...
	if (count($carcasses) > 0){
		...
		foreach ($carcasses as $carcassnum => $creatureid){
			...
			$creature = db_fetch_assoc($result);
			$cleancost = stamina_getdisplaycost("Cleaning the Carcass");
			addnav(array("Clean the carcass of %s (`Q%s%%`0)", $creature['creaturename'], $cleancost), "runmodule.php?module=iitems_meatsystem&op=clean&creatureid=".$creatureid."&carcass=".$carcassnum."&from=".$from);
		}

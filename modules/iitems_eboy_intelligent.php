...
function iitems_eboy_intelligent_getmoduleinfo(){
	$info=array(
...
function iitems_eboy_intelligent_install(){
...
function iitems_eboy_intelligent_uninstall(){
	return true;
}
...
function iitems_eboy_intelligent_dohook($hookname,$args){
	...
	switch($hookname){
		case "village":
			...
			tlschema();
			addnav("eBoy's Trading Station","runmodule.php?module=iitems_eboy_intelligent&op=start");
			break;
		// case "iitems-superuser":
			// output("`n`bIItems: eBoy's Trading Station`b`n");
			// output("`beboy`b: When true, allows the item to be bought and sold at eBoy's Trading Station as long as it resides in the player's 'main' inventory.`n");
			// output("`beboy_dailyadd`b: eBoy's item stock is player-generated.  When a player buys an item from eBoy, the stock goes down by one - when the player sells an item to eBoy, the stock goes up by one.  Use this parameter to automatically add this number of instances of this item to eBoy's shops each day.`n");
			// break;
		case "newday-runonce":
		...
		case "items-returnlinks":
			$args['eboy'] = "runmodule.php?module=iitems_eboy_intelligent&op=start";
			break;
	}
	return $args;
}
	...
	foreach($eboy_info AS $item => $settings){		
		...
		if ($sellable_inventory[$item]){
			...
			if ($sellable_inventory[$item]['quantity'] > 0 && $settings['eboy_price_'.$cid] > 0){
				addnav(array("%s (%s available)",$settings['verbosename'],$sellable_inventory[$item]['quantity']),"runmodule.php?module=iitems_eboy_intelligent&op=sell&item=".$item."&price=".$settings['eboy_price_'.$cid], true);
				addnav("","runmodule.php?module=iitems_eboy_intelligent&op=sell&item=".$item."&price=".$settings['eboy_price_'.$cid]);
				rawoutput("<a href=\"runmodule.php?module=iitems_eboy_intelligent&op=sell&item=".$item."&price=".$settings['eboy_price_'.$cid]."\">Sell for <strong>".number_format($buy)." </strong> Requisition (".$sellable_inventory[$item]['quantity']." available to sell)</a><br />");
			}
		} else {
		...
		if ($session['user']['gold'] >= $settings['eboy_price_'.$cid] && $settings['eboy_stock_'.$cid]){
			addnav("Buy Items");
			addnav(array("%s (%s Req)",$settings['verbosename'],$settings['eboy_price_'.$cid]),"runmodule.php?module=iitems_eboy_intelligent&op=buy&item=".$item."&price=".$settings['eboy_price_'.$cid], true);
			addnav("","runmodule.php?module=iitems_eboy_intelligent&op=buy&item=".$item."&price=".$settings['eboy_price_'.$cid]);
			rawoutput("<a href=\"runmodule.php?module=iitems_eboy_intelligent&op=buy&item=".$item."&price=".$settings['eboy_price_'.$cid]."\">Buy for <strong>".number_format($settings['eboy_price_'.$cid])."</strong> Requisition</a><br />");
		} else {

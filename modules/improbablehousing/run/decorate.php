...
switch(httpget('subop')){
	case "extdesc":
		...
		rawoutput("<form action='runmodule.php?module=improbablehousing&op=decorate&subop=extdesc_change&hid=$hid' method='POST'>Dwelling name: <input name='name' id='name' value=\"".$house['data']['name']."\"><br />Dwelling description:<br /><textarea name='newdesc' id='newdesc'>".$extdesc."</textarea><input type=submit>");
	...
	case "start":
		...
		$roomname=$room['name'];
		rawoutput("<form action='runmodule.php?module=improbablehousing&op=decorate&subop=change&hid=$hid&rid=$rid' method='POST'><input name='name' id='name' value='$roomname'><br /><textarea name='newdesc' id='newdesc'>".$room['desc']."</textarea><input type=submit>");

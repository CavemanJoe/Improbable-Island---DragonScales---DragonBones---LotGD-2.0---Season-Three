...
function rspmutant_round($from, $pchoice)
{
	...
	if ($choice == 1 && $pchoice == 1) { // rock ties rock
		...
		output("`#\"I win,\" `@ whispers the Mutant, staring directly into your eyes and lightly caressing your fingers.");
		...
		output("`#\"Well, it looks like you won this round,\" `@says the Mutant, rubbing his injured fingers.`n`n");
		$playerpoints=$playerpoints+1;
	} elseif ($choice == 1 && $pchoice == 2) { // rock beaten by paper
		output("`@You quickly scribble something on the paper, and hold it out to the Mutant as his crushing rock fist comes hurtling towards your face.  He stops dead to read the paper, upon which you've written the word \"FREAK!\"  Tears well up in his eyes.`n");
		output("`#\"That's really not fair, but I'll concede a defeat,\" `@says the Mutant.`n`n");
		$playerpoints=$playerpoints+1;
		...
		output("`#\"I think I won that round,\" `@says the Mutant.");
		...
		output("`#\"Looks like I won that round,\" `@says the Mutant.");
		...
		output("`#\"Don't worry, I can't actually feel much in that hand.  You win that round,\" `@says the Mutant.`n`n");
		$playerpoints=$playerpoints+1;
	...
	if ($playerpoints==2||$gnomepoints==2){
		...
		output("`n`n`@The mutant returns his hands behind his back. `#\"Time for the next round, I believe.\"");
		rspmutant_nav($from);
	}
}
...
function rspmutant_runevent($type)
{
	...
	if ($op=="" || $op=="search"){
		...
		output("Seeing you, the Mutant begins to hobble towards you.  `#\"Why hello there!  Would you like to play with me?  I'm so desperately lonely.\"`n`n");
		...
		output("`#\"Just choose one of the weapons I have given you, and on the count of three we begin to fight.\"`n`n");
		output("\"`@You've got to be kidding me, right?\"  You look at the meager offerings on display.");
		rspmutant_nav($from);
		...
		$session['user']['specialinc'] = "";
		output("`@You tear off into the Jungle, shouting \"FREAK!\" behind you for good measure.`n`n");
	}
	output_notl("`0");
}
...
function rspmutant_wingame(){
	global $session;
	output("`n`n\"Seems like we have a winner!\" says the Mutant.");
	output("`#\"Now, what can I give you as a prize...\"`@`n`n");
...
function rspmutant_loosegame(){
	global $session;
	output("`n`n`#\"See, I told you I would be a tough contender.\"`@`n`n");

...
function namedmount_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"permanent"=>"How many donator points for free unlimited mount name changes?,int|1000",
		),
		"prefs"=>array(
...
function namedmount_dohook($hookname,$args){
	...
	case "lodge":
		...
		if ($hasperm){
			addnav("Name your Mount (free)","runmodule.php?module=namedmount&op=setname");
		} else {
			addnav(array("Name Your Mount (%s points)", $cost),"runmodule.php?module=namedmount&op=setname");
			addnav(array("Get unlimited Mount Name changes (%s points)", $permcost),"runmodule.php?module=namedmount&op=permanent");
		}
		break;

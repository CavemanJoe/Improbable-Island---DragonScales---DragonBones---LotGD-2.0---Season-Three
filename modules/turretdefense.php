...
function turretdefense_getmoduleinfo(){
	...
	"monstereachchance"=>"The percentage chance for a monster to reach the player after escaping a turret's fire, int|10"
                ),
        "prefs-city"=>array(
...
function turretdefense_run()
{
	...
	switch (httpget('op')){	
		...
		default:
			...
			addnav("Give Cobblestones","runmodule.php?module=turretdefense&op=load");
		...
		case "load":
			...
			rawoutput("<form method=\"post\" action=\"runmodule.php?module=turretdefense&op=loafed&from=".$from."\" name=\"loadstones\">Cobblestones:<input type=\"text\" name=\"givecobblestones\"/><input type=\"hidden\" name=\"submitted\" value=\"true\" /><input type=\"submit\" value=\"Load\" /></form>");
			addnav("", "runmodule.php?module=turretdefense&op=loafed&from=".$from);
		...
		case "loafed":		
			if (($_POST['submitted'] == true) || httppost("submitted")==true)
			{
				require_once "modules/cityprefs/lib.php";
				$givecobbles +=$_POST['givecobblestones'];
				
				if (is_numeric($givecobbles) && $givecobbles > 0)
				{
				//for debugging purposes only, get rid of later
				//output("You are trying to give %s cobbles, you have %s points and have spent %s`n`n",$givecobbles,$wcgpointsava,$wcgspent);
				if ($givecobbles <=($wcgpointsava/*-$wcgspent*/)){
				...
				//$cid = get_cityprefs_cityid("location",$session['user']['location']);
					...
					switch ($session['user']['location']){
						 ...
						 case "AceHigh": output("You put %s cobblestones down.  It is immediatly sorted into a series of small piles. Looking around, you see cobblestones only in small piles. What the jokers use them for, you're not sure, but you're also not sure if you `iwant`i to know.`n`n",$givecobbles); break;
			 			default:output("You put %s cobblestones down, to be turned into ammuntion.`n`n",$givecobbles);break;
						}
					addnav("What now?");
				} else {
					output("You try putting down %s cobbles, but realise you don't have that many. Please try again.",$givecobbles);
					addnav("Oops!");
					addnav("Try Again","runmodule.php?module=turretdefense&op=load");
				}
				} else {
					output("Sorry, the data you entered is invalid. Please try again.");
					addnav("Oops!");
					addnav("Try Again","runmodule.php?module=turretdefense&op=load");
		...
		case "fire":
			...
			$lv = onslaught_checkmonsters();
				...
				if ($lv>=100 && $def<$num && $stopchance > 80){
					...
					output("How many cobbles will you load? You see there are %s lying here.",$cobbles);
					rawoutput("<form method=\"post\" action=\"runmodule.php?module=turretdefense&op=fight&from=".$from."\" name=\"loaded\"><input type=\"text\" name=\"loadcobblestones\" /><input type=\"hidden\" name=\"loadcobbles_submitted\" value=\"true\" /><input type=\"submit\" value=\"Fire!\"/></form> ");
					addnav("","runmodule.php?module=turretdefense&op=fight&from=".$from);				
			}
			addnav("Hang around on the walls","runmodule.php?module=turretdefense");			
			addnav("Return to Outpost","village.php");break;
			
			
		case "fight":
			if ($_POST['loadcobbles_submitted'] == true){
				$loadedcobbles = $_POST['loadcobblestones'];
				output("You are trying to fire %s cobbles",$loadedcobbles);
				if (is_numeric($loadedcobbles) && $loadedcobbles>0){
				
                                    $cobbles = turretdefense_checkcobbles();
				if ($loadedcobbles <= $cobbles){
					$minload = get_module_setting("mincobbles");
					
					output(" and the minimum is %s",$minload);
					output("Less cobbles loaded than actally there.");////
					if ($loadedcobbles >= $minload){
					output("More than the minimum amount of cobblestones loaded");////
					$maxload = get_module_setting("maxcobbles");
					if ($loadedcobbles <= $maxload || $maxload==-1){
						output("Less cobblestones loaded than the max, or max = -1");////
						$cid = get_cityprefs_cityid("location",$session['user']['location']);
						$oldcitycobbles = turretdefense_checkcobbles();
			 	...
			 	case "Improbable Central": 
			 		...
			 		//There may be some way of getting the switch to work, otherwise I'll just use the if statements.
		...
		if ($victory){
			...
			fightnav(true,true);
					...
					if ($session['user']['race'] == Human)
			 		...
			 		if ($session['user']['race'] == Kittymorph)
			 		{
			 			output("You fire your bow at the oncoming horde, which just goes straight into the ground.`n`n");
			 			break;
			 		}
			 		if ($session['user']['race'] == Mutant)
			 		{
			 ...
			 default:output("You put a load of cobbles into the turret, put are stopped by one of the other guards. \"`qDon't put so many in there,`0\" he says. \"`qWe only have a limited supply, so try to make them last.`0\"`n`n");break;
				...
				output("Oops! You didn't put in any cobbles or at least not enough to make a difference!");
				addnav("Try again!");
				addnav("Reload","runmodule.php?module=turretdefense&op=fire");
				}				
			...
			output("You didn't put in a number.");
			addnav("Reload","runmodule.php?module=turretdefense&op=fire");
			}
			}else{
			output("You didn't put in a number.");

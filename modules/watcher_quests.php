	...
	switch($hookname){
		case "village":
			...
			if ($session['user']['level'] > 2 && get_module_pref("plotpoint1")==0){
				...
				redirect("runmodule.php?module=watcher_quests&plotpoint=1");
			}
			...
			if ($session['user']['dragonkills'] >= 3 && get_module_pref("plotpoint2")==0 && $session['user']['gems'] > 1){
				redirect("runmodule.php?module=watcher_quests&plotpoint=2");

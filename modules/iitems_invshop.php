...
function iitems_invshop_getmoduleinfo(){
	$info=array(
		"name"=>"IItems - Inventory Item Shop",
		"version"=>"2009-06-20",
...
function iitems_invshop_run(){
	...
	//establish what the player already has, and get its details
	$pinv = iitems_get_player_inventory();
	foreach($pinv AS $key => $vals){
		if ($vals['wlimit_capacity']){
			$old[$vals['inventorylocation']] = $vals['itemid'];
		}
	}
	$old_main = iitems_get_item_details($old['main']);
	$old_fight = iitems_get_item_details($old['fight']);
	$tradein_main = round($old_main['invshop_price'] * 0.6);
	$tradein_fight = round($old_fight['invshop_price'] * 0.6);
	...
	switch (httpget('op')){
		...
		case "buy":
			...
			$itemdetails = iitems_get_item_details($item);
			
			if ($itemdetails['invshop_price'] > ($session['user']['gems'] + $tradein)){
				//todo
				...
				if ($has || $has===0){
					...
					$session['user']['gems'] -= $itemdetails['invshop_price'];
					$price = $itemdetails['invshop_price'] - $tradein;
					debug($item);
					iitems_give_item($item);

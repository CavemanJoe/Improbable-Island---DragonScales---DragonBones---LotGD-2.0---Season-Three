...
/*
We want this to be ready for:
High priority:
	AJAX
		Needs a function that returns commentary in an array, which is then passed to a function that formats it for the page and another function that formats it for the AJAX bits
	Icons
		Icons built into the commentary table via serialized "info" array
	Styling / Theming
	Clan data stored in the commentary table saves lookups
	Per-Post Avatars (LJ-style, or perhaps emoticon-style)
		Serialized "info" array
	HTML Export
	New output system
		So we can get rid of all this tlschema shite
	Special comments
		Use a "skipname" value in the info array
Low priority:
	User-selectable default colour code
*/
...
// *** DRAGONBG.COM CORE PATCH END ***
	...
	// *** DRAGONBG.COM CORE PATCH START ***	
		$sql = "SELECT COUNT(commentid) AS newadded FROM " .
			...
			db_prefix("commentary"). ".author WHERE " . ($section ? "section='$section' AND " : '') .
